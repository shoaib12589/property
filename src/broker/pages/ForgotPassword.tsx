...
import forgotRightBg from '../../agent/assets/agent-login-left.png'
  ...
  return (
    <div className="min-h-screen min-h-[100dvh] grid grid-cols-1 lg:grid-cols-2 w-full overflow-x-hidden">
      ...
      >
        <div className="w-full max-w-[400px] mx-auto text-left">
          ...
          <form
            ...
            onSubmit={(e) => {
              e.preventDefault()
              navigate('/broker/otp-verification', {
                state: { email: email.trim() || undefined },
              })
            }}
          >
            <div>
              <label className="block text-sm font-semibold mb-1.5" style={{ color: tokens.textField }}>
                Email Address
              </label>
            ...
            <Button
              type="submit"
              className="w-full h-11 sm:h-12 text-white font-semibold text-sm sm:text-base hover:opacity-95 min-h-[44px] rounded-none"
      ...
      <div className="hidden lg:block relative min-h-[50vh] lg:min-h-screen">
        <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url(${forgotRightBg})` }} />
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[90%] max-w-[520px] px-2">
          ...
          >
            ...
            <p
              className="text-center text-[32px] max-w-xl mx-auto"
              style={{ fontFamily: "'Arial', sans-serif", fontWeight: 400, color: 'rgba(255, 255, 255, 0.95)', lineHeight: 1.5 }}
            >
