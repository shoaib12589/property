...
const SETTINGS_ITEMS: AccordionItem[] = [
  {
    ...
    label: 'Account',
    type: 'content',
    content: `Manage your account email, password, and security preferences from your profile.`,
  },
  {
    id: 'email',
  ...
  },
  {
    ...
    label: 'Agency',
    type: 'content',
    content: `Configure agency-wide defaults, branding, and team permissions.`,
  },
  {
    id: 'listings',
    label: 'Listings',
    type: 'content',
    content: `Set defaults for new listings, syndication, and visibility options.`,
  },
  {
    id: 'calendar',
    label: 'Calendar',
    type: 'content',
    content: `Sync your calendar and choose default availability for showings.`,
  },
  {
    id: 'messages',
    label: 'Messages',
...
function AccordionRow({
  ...
  onToggleOption: (key: string, defaultEnabled: boolean) => void
}) {
  ...
  return (
    <div className="border border-[#E5E7EB] rounded-lg bg-white overflow-hidden">
      ...
      >
        ...
        {isExpandable ? (
          <ChevronDown
            className={['w-5 h-5 text-gray-400 transition-transform duration-200 shrink-0', open ? 'rotate-180' : ''].join(' ')}
          />
        ) : (
            ...
            {item.toggles.map((t) => (
              <div key={t.key} className="flex items-center justify-between gap-4">
                ...
                <Toggle
                  ...
                  onChange={() => onToggleOption(t.key, t.enabled)}
                />
              </div>
            ))}
          </div>
      ...
      {open && item.type === 'content' && item.content && (
        <div className="px-5 pb-4 border-t" style={{ borderColor: tokens.cardBorder }}>
          <div className="pt-3">
            <p className="text-sm text-[#374151] whitespace-pre-line leading-relaxed" style={font}>
              {item.content}
            </p>
          </div>
        </div>
...
export function Settings() {
  ...
  const handleToggleOption = (key: string, defaultEnabled: boolean) => {
    setToggleState((prev) => {
      const cur = prev[key] ?? defaultEnabled
      return { ...prev, [key]: !cur }
    })
  }
